...
|---|---|
| Branch | `main` (clean) |
| Tests | **770 passing / 0 fail** (`npm test`) |
| Lint | clean (`npx eslint@9 --max-warnings=0 .`) |
| SW cache | `hermantrip-shell-v82` (in `sw.js`) |
| Latest tag | `v0.M66.3` (M5 + 3 slideshow-UX hotfixes). Next milestone = **M6 → `v0.M67`** |
| Next SW bump | **v83** |
...
Tags continue the ladder — **before tagging, run `git tag --list 'v0.*'`** to confirm the next number. Sub-fixes use `v0.M66.N` (next would be `v0.M66.4`); they do NOT consume the next milestone number (M6 is `v0.M67`).
...
# bump sw.js SHELL_CACHE (v82→v83) + add any NEW src/lib|views file to SHELL_FILES
...
**Branch `m6-map-globe`, tag `v0.M67`, SW v83.** **STOP and ask the owner first:** M6 needs the **MapTiler domain-restricted key + the light/dark Hebrew style IDs**, pasted into a new `src/config.js` (the key is intentionally NOT in the repo). Don't start the map work until you have them.
...
Then **M7 Game** (`v0.M68`, SW v84) and **M8 Timeline** (`v0.M69`, SW v85 — data-dependent scrubber; visually sanity-check against real manifest data, not only unit tests).
