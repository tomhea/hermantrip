...
Last updated after **M7 (Game) + its three owner-feedback hotfixes shipped & live**
(`v0.M68`…`v0.M68.3`, SW v91);
**next up — and LAST — is M8 (Timeline) → `v0.M69`; next SW bump = v92** (the
next thing to ship takes it, milestone or hotfix).
⏳ One open game follow-up (owner-gated, NOT blocking M8): confetti currently
fires on EVERY done screen (`GAME_CONFETTI_PREVIEW_ALL=true` in `main.js`) so the
owner can approve it; once they do, flip that flag to `false` (confetti = 20/20
only) as a one-line hotfix._
...
header, and put PC corner labels on a solid scrim chip.
...
|---|---|
| Branch | `main` (clean) |
| Tests | **808 passing / 0 fail** (`npm test`) |
| Lint | clean (`npx eslint@9 --max-warnings=0 .`) |
| SW cache | `hermantrip-shell-v91` (in `sw.js`) |
| Latest tag | `v0.M68.3` (M7 Game + 3 hotfixes). Next milestone = **M8 → `v0.M69`** (the LAST one) |
| Next SW bump | **v92** |
...
**Ship:** TDD task-by-task (failing `*.test.mjs` → implement → pass; capture FAIL+PASS logs) → bump `sw.js` SHELL_CACHE **v91→v92** + **add any NEW `src/lib`/`src/views` module to `SHELL_FILES`** → `npm test` (0 fail) → `npx eslint@9 --max-warnings=0 .` → PR `M8: Timeline` with the R1 + R2 (3 viewports incl. tablet 820 + console-clean probe) + R5 (gzip delta) body → crist (APPROVED only) → artifact `versions/v0.M69/dist-M69.tar.gz` → crist re-review → `gh pr merge --merge` → tag `v0.M69` → deploy → verify-live → branch cleanup (full sequence in §2). **Timeline is data-dependent: visually sanity-check the rendered scrubber against real manifest data** (does תאילנד recur at the right spots? are the segment widths ~proportional to the time spent?), not only via unit tests. It's the LAST milestone — after it ships, the ladder M1–M8 is complete.
