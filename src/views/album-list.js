...
function backHeader(title, subtitle) {
  return `
    <header class="view-header">
      <a class="back-link" href="#/" aria-label="חזרה לדף הבית">→ דף הבית</a>
...
export function renderAlbumList({ manifest, error, code, dpr = 1 }) {
  ...
  if (!country) {
    return `
      ...
      <p class="muted">המדינה המבוקשת לא נמצאה. <a href="#/">חזרה לדף הבית</a>.</p>
    `;
  }
  ...
  return `
    ...
    <ul class="album-grid-list" aria-label="אלבומים">
      ${albums.map((a) => albumCard(a, dpr)).join('')}
    </ul>
  `;
}
